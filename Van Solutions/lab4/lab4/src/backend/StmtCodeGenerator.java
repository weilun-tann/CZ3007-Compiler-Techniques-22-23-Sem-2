...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		WhileStmt outerScope = nd.getEnclosingLoop();
		Unit exitWhile = breakTargets.get(outerScope);
		units.add( j.newGotoStmt(exitWhile) );
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		 ...
		 */
		NopStmt startWhile = j.newNopStmt();
		NopStmt exitWhile = j.newNopStmt();

		breakTargets.put(nd, exitWhile);
		
		units.add(startWhile);

		Value cond = ExprCodeGenerator.generate(nd.getExpr(), fcg);
		units.add(j.newIfStmt(j.newEqExpr(cond, IntConstant.v(0)), exitWhile));

		nd.getBody().accept(this);
		
		units.add(j.newGotoStmt(startWhile));
		units.add(exitWhile);
