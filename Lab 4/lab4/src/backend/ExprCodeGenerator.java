...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	/**
	 * Ensures that the given value can be used as an operand; that is, if the value is not a
	 * {@link Local} or a {@link Constant}, this method allocates a new temporary variable and
	 * stores the value into that temporary.
	 */
	...
	public Value visitBooleanLiteral(BooleanLiteral nd) {
		/*
		 * TODO: return something meaningful here (hint: translate 'true' to integer constant 1,
		 * 'false' to integer constant 0)
		 */
	...
	public Value visitArrayLiteral(ArrayLiteral nd) {
		...
		for (int i = 0; i < nd.getNumElement(); ++i) {
			...
			units.add(
					Jimple.v().newAssignStmt(Jimple.v().newArrayRef(array, IntConstant.v(i)), elt));
		}
		return array;
	}
	...
	public Value visitVarName(VarName nd) {
		...
		if (decl.isLocal()) {
			...
			return Jimple.v().newStaticFieldRef(
					Scene.v().makeFieldRef(declaringClass, decl.getName(), fieldType, true));
		}
	}
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		/*
		 * TODO: generate code for binary expression here; you can either use a visitor to determine
		 * the type of binary expression you are dealing with, or generate code in the more
		 * specialised visitor methods visitAddExpr, visitSubExpr, etc., instead
		 */
	...
	public Value visitCall(Call nd) {
		...
		SootClass calleeSootClass =
				fcg.getModuleCodeGenerator().getProgramCodeGenerator().getSootClass(calleeModule);
		SootMethodRef callee =
				Scene.v().makeMethodRef(calleeSootClass, calleeName, parmTypes, rettp, true);
