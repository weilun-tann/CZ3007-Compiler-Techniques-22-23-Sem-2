...
/**
 * This class is in charge of creating Jimple code for a given statement (and its nested statements,
 * if applicable).
 */
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		/*
		 * TODO: generate code for break statement (hint: use ASTNode.getEnclosingLoop and
		 * breakTargets; use units.add() to insert the statement into the surrounding method)
		 */
		WhileStmt whileStmt = nd.getEnclosingLoop();
		Unit unit = breakTargets.get(whileStmt);
		units.add(j.newGotoStmt(unit));
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		/*
		 * TODO: generate code for while statement as discussed in lecture; add the NOP statement
		 * you generate as the break target to the breakTargets map
		 */
		...
		units.add(j.newIfStmt(j.newEqExpr(condition, IntConstant.v(0)), exitLoop));
